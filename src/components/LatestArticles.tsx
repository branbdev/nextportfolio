...
const LatestArticles: React.FC = () => {
  const [fallbackArticles, setFallbackArticles] = useState<BlogPost[]>([]);
  const [fallbackLoading, setFallbackLoading] = useState<boolean>(false);
  
  // Try to use GraphQL first
  const { loading: graphqlLoading, error, data } = useQuery(GET_LATEST_BLOG_POSTS, {
    variables: { count: 3 },
    // If there's an error, fall back to direct function call
    onError: () => {
      setFallbackLoading(true);
      // Fallback to hardcoded data if we're on the client side
      setFallbackArticles([
        {
  ...
  const loading = graphqlLoading || fallbackLoading;
  const articles = data?.latestBlogPosts || fallbackArticles;
            ...
            <div className="blog_list_items">
              {articles.map((article, index) => (
                <div key={index} className="blog_item">
                  <span className="blog_date">{new Date(article.date).toLocaleDateString()}</span>
