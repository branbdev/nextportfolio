...
import { GET_ALL_TECHNOLOGIES, GET_TECHNOLOGY_BY_SLUG } from '../../../src/lib/graphql/operations';
...
export default async function TechnologyPage(props: { params: Promise<Params> }) {
  const { slug } = await props.params;
  const { data } = await graphqlRequest<{ technology: any }>(print(GET_TECHNOLOGY_BY_SLUG), { slug });
    ...
    <main className="container mx-auto py-8">
      <header className="mb-8">
        ...
        {tech.logo ? (
          ...
          <img src={tech.logo} alt={`${tech.name} logo`} className="mt-4 h-16 w-16 object-contain" />
        ) : null}
      </header>
          ...
          <ul className="space-y-3">
            {tech.relatedPosts.map((p: any) => (
              <li key={p.slug} className="border-b pb-3">
                <a href={`/blog/${p.slug}`} className="text-lg font-medium underline">
                  {p.title}
                </a>
          ...
          <ul className="space-y-3">
            {tech.relatedProjects.map((prj: any) => (
              <li key={prj.slug} className="border-b pb-3">
                <a href={`/projects/${prj.slug}`} className="text-lg font-medium underline">
                  {prj.name}
                </a>
